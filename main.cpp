...
struct RegisterFile {
    ...
    unordered_map<string,string> varToReg; // var -> reg
    unordered_map<string,string> regToVar; // reg -> var

    string spillReg = ""; // optional
    ...
    string alloc(const string &var) {
        ...
        // no free register → spill the first
        string victim = regs[0];
        string victimVar = regToVar[victim];

        // spill victim to memory
        spillReg = victimVar;
        // caller will emit sw for spill later

        varToReg.erase(victimVar);
        regToVar.erase(victim);

        // now assign to new var
        varToReg[var] = victim;
        regToVar[victim] = var;
        return victim;
    }
...
class CodeGen {
    vector<string> out;
    RegisterFile rf;
    unordered_map<string,int> dataOffsets; // label -> reserved (not needed but keep)
...
public:
    void generate(AST::Function &fn) {
        // collect variables from VarDecl and Assign targets
        collectVars(fn);
        ...
        out.push_back("main:");
        // prologue: not strictly required for this tiny compiler
        // generate body
        for (auto &stmt : fn.body) genStmt(stmt.get());
        // epilogue: return
        out.push_back("li $v0, 10  # exit syscall");
...
private:
    void collectVars(AST::Function &fn) {
        ...
        // traverse statements and gather variable names
        function<void(AST::Node*)> walk = [&](AST::Node* node) {
            ...
            if (auto vd = dynamic_cast<AST::VarDecl*>(node)) {
                ...
                // variable used but not declared create it as well
                if (!seen[v->name]) {
    ...
    void genStmt(AST::Node *node) {
        if (auto vd = dynamic_cast<AST::VarDecl*>(node)) {
            // evaluate init into $t0 and store to label
            genExpr(vd->init.get(), "$t0");
            emit("la $t1, " + vd->name);
            emit("sw $t0, 0($t1)");
            return;
        }
        if (auto as = dynamic_cast<AST::Assign*>(node)) {
            genExpr(as->expr.get(), "$t0");
            emit("la $t1, " + as->name);
            emit("sw $t0, 0($t1)");
            return;
        }
        ...
        if (auto r = dynamic_cast<AST::ReturnStmt*>(node)) {
            genExpr(r->expr.get(), "$t0");
            // place return in $v0 and exit
            emit("move $v0, $t0");
            emit("li $v0, 1    # print int syscall (optional)");
            emit("move $a0, $t0");
            emit("syscall");
            // exit program
            emit("li $v0, 10");
    ...
    // generate code to evaluate expr into 'dstReg' (e.g. $t0)
    void genExpr(AST::Node *node, const string &dstReg) {
        ...
        if (auto bin = dynamic_cast<AST::Binary*>(node)) {
            ...
            if (op == "&&") {
                ...
                genExpr(bin->left.get(), "$t1");
                emit("beq $t1, $zero, " + Lfalse);
                genExpr(bin->right.get(), "$t2");
                emit("beq $t2, $zero, " + Lfalse);
                emit("li " + dstReg + ", 1");
                emit("j " + Lend);
                emit(Lfalse + ":");
                emit("li " + dstReg + ", 0");
            ...
            if (op == "+") emit("add " + dstReg + ", $t1, $t2");
            else if (op == "-") emit("sub " + dstReg + ", $t1, $t2");
            else if (op == "*") emit("mul " + dstReg + ", $t1, $t2");
            else if (op == "/") {
                emit("div $t1, $t2");
                emit("mflo " + dstReg);
            }
            else if (op == "<") emit("slt " + dstReg + ", $t1, $t2");
            else if (op == ">") emit("slt " + dstReg + ", $t2, $t1");
            else if (op == "==") {
                string Ltrue = genLabel("Leq_true");
                string Lend = genLabel("Leq_end");
                emit("beq $t1, $t2, " + Ltrue);
                emit("li " + dstReg + ", 0");
                emit("j " + Lend);
                emit(Ltrue + ":");
                emit("li " + dstReg + ", 1");
                ...
                throw runtime_error("Unsupported binary op: " + op);
            }
            return;
        }
    ...
    void genCondJumpFalse(AST::Node *cond, const string &labelFalse) {
        // evaluate cond into $t0
        genExpr(cond, "$t0");
        emit("beq $t0, $zero, " + labelFalse);
    }
};
